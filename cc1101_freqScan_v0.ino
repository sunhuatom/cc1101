...
#define MIN_DBM -120
#define STEP_DBM 10
#define RSSI_DELAY 1200 //rssi delay in micro second
...
uint8_t lastChanne[NUM_OF_SUB_BANDS] ={255,255,255};
...
uint8_t activeBand ; //store subband that contains strongest signal
uint16_t activeChannel; //
uint8_t calCounter=0; //to deterin when to calibrate
...
void scanFreq(void){
  ...
  for(subBand = 0; subBand<NUM_OF_SUB_BANDS;subBand++){
      ...
      for (channel=0;channel<=lastChanne[subBand];channel++){
        uint8_t pktStatus;
